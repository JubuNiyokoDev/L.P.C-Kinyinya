...
def update_data():
    ...
    query="update ListStudent_lng3 set Name=?,Mobile=?,Email=?,Address=?,Gender=?,D_O_B=?,T_O_B=?,Added_Time=?,Added_Date=? where Id=?"
...
def show_student():
    connection = sqlite3.connect("school.db")
    mycursor = connection.cursor()
    query="SELECT * FROM ListStudent_lng3"
...
def delete_student():
    ...
    query='delete from ListStudent_lng3 where id=?'
...
def search_data():
    ...
    connection = sqlite3.connect("school.db")
    mycursor= connection.cursor()
    query = ("select * from ListStudent_lng3 where Id=? or Name=? or Mobile=? or  Email=?  or Address=? or Gender=? or D_O_B=? or T_O_B=?")
...
def add_data():
    if idEntry.get()=='' or  nameEntry.get()=='' or  phoneEntry.get()=='' or  emailEntry.get()=='' or  adressEntry.get()==''  or  genderEntry.get()=='' or  dobEntry.get()=='':
        ...
        try:
            connection = sqlite3.connect("school.db")
            mycursor= connection.cursor()
            mycursor.execute("INSERT INTO ListStudent_lng3 (Id,Name,Mobile,Email,Address,Gender,D_O_B,T_O_B,Added_Time,Added_Date) VALUES(?,?,?,?,?,?,?,?,?,?)",(idEntry.get(),nameEntry.get(),phoneEntry.get(),emailEntry.get(),adressEntry.get(),genderEntry.get(),dobEntry.get(),tobEntry.get(),currenttime,date))
            connection.commit()
...
def connect():
     ...
     try :
         connection = sqlite3.connect("school.db")
         mycursor = connection.cursor()
         mycursor.execute("CREATE TABLE IF NOT EXISTS ListStudent_lng3(Id int,Name TEXT,Mobile TEXT,Email TEXT,Address TEXT,Gender TEXT,D_O_B TEXT, T_O_B TEXT,Added_Time TEXT,Added_Date TEXT)")
         connection.commit()
     except :
         messagebox.showerror("Error","Sorry Something went wrong")
...
s ="Student Managment System Lnague 3"
